'''
NOTES FROM OH:
- run multiple lines (but not the whole thing) with: highlight+command+enter
- if having issues always install any updates first
- use interupt python for never ending scripts
- use subsets of enormous data sets if commands are running endlessly
- pset3 will take 4-5hrs to run
'''
...
bigdf_noedits = pd.read_json('data/tweets_80000.json')
